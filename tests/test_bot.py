...
#
from exchange.bot.bot import Api, Bot, Currency, User
...
START_MONEY = settings.start_money
...
def _bot_mock_register_user(mocker, user):
    mocker.patch.object(Api, 'register_user')
    Api.register_user.return_value = user


@pytest.fixture()
def _bot_patch_get_currency(mocker):
    mocker.patch.object(Api, 'get_currency')


@pytest.fixture()
def _bot_mock_get_currency(currency, _bot_patch_get_currency):
    Api.get_currency.return_value = currency
...
@pytest.mark.usefixtures('_bot_mock_register_user', '_bot_mock_get_currency')
def test_first_buy(mocker, bot):
    mocker.patch.object(Api, 'buy')
    assert bot.first_buy().name == 'test'


def test_bot_make_request_with_invalid_http_method():
    with pytest.raises(TypeError):
        Api.make_request('google.com', None)
...
def test_bot_make_request_when_it_get_error_in_response(requests_get_mock):
    ...
    with pytest.raises(ApiResponseException):
        Api.make_request('google.com', HTTPMethod.GET)
...
def test_first_buy_when_time_was_change(mocker, bot):
    mocker.patch.object(Api, 'buy')
    Api.buy.side_effect = [
        ApiResponseException('currency price was change, check new price'),
        None,
    ]
    assert bot.first_buy().name == 'test'
...
@pytest.mark.usefixtures(
    '_bot_mock_register_user', '_bot_mock_get_multiple_currencies_currency_growing'
)
def test_bot_monitoring(mocker, bot):
    mocker.patch.object(Api, 'buy')
    mocker.patch.object(Api, 'sell')
    bot.start()
    assert bot.bot_finished


@pytest.mark.usefixtures(
    '_bot_mock_get_multiple_currencies_currency_falls', '_bot_mock_register_user'
)
